...
/*   Updated: 2025/04/02 17:50:27 by vihane           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*Permet de remplacer une clef (variable $VAR ou $?) dans la
ligne de commande (*line) par sa valeur (value) strjoin concatène
la valeur avec la partie de la ligne qui suit la clef,
is failed malloc vérifie si l'allocation de mémoire pour new_line
a réussi. head permet de stocker la ligne qui précède la clef
to_dollar représent l'indice où commence la clef dans la ligne.
strlcpy : on copie une chaine de caractère dans une autre, tout
en garantissant que la destination, ici head auquel on a allouer
la place nécessaire, n'overflowed pas. to dollar, contient l'indice
du commencement de la var donc si la var commence à 5, head a une place
de 5 et donc avec strlcpy on copie le début de la ligne dans head.*/
