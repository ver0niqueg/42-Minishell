...
/*   Updated: 2025/04/02 00:27:55 by vgalmich         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*Structure qui stockent les informations lies au parsing selon la ligne 
de commande entree*/
...
void	expand_nbr(char **line, t_point expand, int *new_len,
			t_minishell *minishell);
void	look_for_var(char **line, int *new_len, int *start,
			t_minishell *minishell);
...
void	add_command(t_line_info line_info, int *cmd_size,
			t_parsing *parsed, t_minishell *minishell);
void	parsing(t_parsing *parsed, char *line, t_minishell *minishell,
			int init);
...
int		handle_heredoc(char *line, int start, t_parsing *parsed,
			int *limiter_size);
...
void	syntax_err_msg(t_parsing *parsed, char first, char second,
			char third);
...
int		wrong_data_type(t_check_path *cp, t_parsing *parsed, char *exp,
			int end);
...
int		ft_set_env(char ***env, const char *var, const char *value,
			int overwrite);
...
void	cmd_path_error(const char *msg, char *path, t_parsing *parsing_data,
			int nb);
int		validate_file_access(t_check_path *cp, char *path,
			t_parsing *parsing_data);
int		analyze_path_and_perms(t_check_path *cp, char *path,
			t_parsing *parsing_data);
...
void	redirect_input_from_pipe(int i, t_minishell *minishell,
			t_pipes *pipes, pid_t *pids);
int		set_input_redirs(int i, t_minishell *minishell,
			t_pipes *pipes, pid_t *pids);
...
int		set_output_redirs(int i, t_minishell *minishell,
			t_pipes *pipes, pid_t *pids);
...
void	handle_cmd_exec(int i, t_minishell *minishell,
			t_pipes *pipes, pid_t *pids);
