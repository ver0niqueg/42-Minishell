...
void	cmd_error(int i, t_minishell *minishell, int *fd_out, int *fd_in)
{
	...
	free_parsed(minishell->parsed, minishell->nb_of_cmds); // code free all parsed
...
int	cmd_null(int i, t_minishell *minishell)
{
	if (minishell->parsed[i].cmd == NULL
		&& minishell->parsed[i].err_nb == 0) // aucune erreur detecte pour cette cmd nulle
		return (1);
	return (0);
}
...
void	handle_cmd_exec(int i, t_minishell *minishell, t_pipes *pipes, pid_t *pids)
{
	...
	if (minishell->parsed[i].err_nb != 0) // si une erreur a ete detecte
