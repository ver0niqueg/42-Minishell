...
/*   Updated: 2025/04/02 17:59:15 by vihane           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_init_line(char *line, t_minishell *minishell)
{
	...
	minishell->parsed = (t_parsing *)malloc(sizeof(t_parsing)
			* (minishell->nb_of_cmds));
	if (!minishell->parsed)
	{
	...
	ft_memset(minishell->parsed, 0, sizeof(t_parsing)
		* (minishell->nb_of_cmds));
}
...
/*On analyse la ligne graduellement, d'abord en prenant en compte si les 
premieres quotes sont bien fermées, ensuite en initialisant se dont 
on a besoin et enfin gérant le cas si il y a une erreur de syntaxe ou pas.
